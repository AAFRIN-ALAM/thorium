...
const createReview = async function (req, res) {
    try {
        ...
        // check reviewedBy is present
        if (!reviewData.reviewedBy) res.status(400).send({ status: false, msg: "reviewedBy is required" })

        if (Object.keys(reviewData).includes('reviewedBy')) {     //includes return the boolean value
...
const updateReview = async function (req, res) {
    try {
        ...
        book = book.toObject()    // Object.assign({},book)
